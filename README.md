...
- **Click a planet** for its dependents and imports (tagged `type`, `lazy` or `cycle`), instability, lines of code,
  size, commits in the last year, unresolved imports and the shortest cycle through it.
...
In the map: `/` searches, `Esc` resets the view, drag to orbit, scroll to zoom and right-drag to pan.
